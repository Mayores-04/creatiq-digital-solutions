...
It creates the CRM tables, RLS policies, published-site policies, private `creatiq-digital-solutions` bucket, public-service and portfolio seeds, and the Staff-only operational RPCs.
...
Never expose an Owner invite or promotion flow on the public site.

## 3. Configure environment variables

Copy the root `.env.example` into `.env.local` and fill the values from the Supabase project, Cloudinary, and SMTP provider. The `SUPABASE_SERVICE_ROLE_KEY` must be the actual **service_role** secret from Supabase’s API settings—not a publishable/anon key. It is used only for secure Owner-created Staff invitations; website inquiries use a dedicated, validated database RPC.

## 4. Confirm security

- Anonymous visitors can only read published company settings, services, and portfolio projects.
...
- Owners have the administrative CRUD and publishing permissions.
