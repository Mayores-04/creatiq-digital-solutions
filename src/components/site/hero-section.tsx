...
import { Activity, Code2, Smartphone } from "lucide-react";
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-7xl items-center gap-10 px-4 py-16 sm:px-6 sm:py-20 lg:min-h-[calc(100vh-6rem)] lg:grid-cols-2 lg:gap-14 lg:px-10 xl:px-16">
        <div className="max-w-xl space-y-6">
          <div className="animate-fade-in-up inline-flex max-w-full items-center gap-2.5 rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3.5 py-1.5 shadow-[0_0_24px_rgba(8,189,255,0.12)] backdrop-blur-md">
            ...
            />
            <span className="truncate text-[9px] font-bold uppercase tracking-[0.18em] text-secondary sm:tracking-[0.22em]">
          ...
          <div className="animate-fade-in-up delay-200 grid grid-cols-3 gap-2 pt-2 sm:gap-3">
            {highlights.map(([title, description]) => (
              <div
                key={title}
                className="min-w-0 rounded-2xl border border-cyan-300/15 bg-white/[0.03] p-3 backdrop-blur-md sm:p-3.5"
              >
                <p className="text-lg font-black text-secondary sm:text-xl">
                  {title}
                </p>
                <p className="mt-1 text-[9px] leading-4 text-muted sm:text-[10px]">
                  {description}
                </p>
              </div>
            ))}
  ...
  return (
    <div className="relative mx-auto hidden min-h-[480px] w-full max-w-xl items-center justify-center lg:flex">
      ...
      <div className="floating relative w-full max-w-[455px]">
            ...
            <div className="grid gap-3 p-4">
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div
                    key={label}
                    className="min-w-0 rounded-2xl bg-cyan-300/10 p-3"
                  >
                    <p className="truncate text-[9px] uppercase tracking-widest text-muted">
                      {label}
                    </p>
              ...
              <div className="rounded-2xl border border-white/10 bg-background/50 p-4">
                ...
                <div className="space-y-2.5">
                  {["w-[86%]", "w-[72%]", "w-[94%]"].map((width) => (
                    <div
                      key={width}
                      className="h-2.5 w-full rounded-full bg-white/10"
                    >
                      <div
                        className={`h-2.5 ${width} rounded-full bg-gradient-to-r from-blue-600 to-cyan-300`}
                      />
                    </div>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <ShowcaseCard
                  icon={<Code2 size={22} />}
                  title="Web Systems"
                  description="Fast, responsive, and scalable platforms."
                />

                <ShowcaseCard
                  icon={<Smartphone size={22} />}
                  title="Mobile Apps"
                  description="Smooth interfaces for modern users."
                />
              </div>
            </div>
          </div>
        ...
        <div className="glass-card absolute -bottom-24 -left-5 z-20 w-[255px] rounded-2xl p-4 shadow-[0_0_45px_rgba(8,189,255,0.18)]">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-base font-black tracking-[-0.03em] text-primary">
                Creatiq
              </p>
              <p className="mt-0.5 text-[10px] font-semibold tracking-[0.08em] text-secondary">
                Create. Innovate. Elevate.
              </p>
            </div>
          </div>

          <div className="my-3 h-px w-full bg-gradient-to-r from-transparent via-cyan-300/30 to-transparent" />

          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <span className="h-2 w-2 rounded-full bg-secondary shadow-[0_0_12px_rgba(8,189,255,0.8)]" />
              <span className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted">
                Digital Studio
              </span>
            </div>

            <span className="rounded-full bg-cyan-300/10 px-2 py-1 text-[9px] font-bold uppercase tracking-widest text-secondary">
              Active
            </span>
          </div>
        </div>
...
function ShowcaseCard({
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
      <span className="mb-3 block text-secondary">{icon}</span>
      <p className="text-sm font-bold text-primary">{title}</p>
      <p className="mt-1.5 text-[11px] leading-5 text-muted">{description}</p>
    </div>
  );
}
